...
from utils.util import check, update_linear_schedule
...
class MAPPOPolicy:
    ...
    def get_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic,
                    masks, context, available_actions=None, deterministic=False):
        # 这两个函数仅用于执行, context: (n_envs, z_dim)
        obs = ptu.from_numpy(obs)
        x_actor = torch.cat([obs, context], dim=-1)
    ...
    def evaluate_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic,
        ...
        obs = ptu.from_numpy(obs)  # obs: [env_num*max_ep_len, obs_dim]
        z_actor = z_l[:, self.agent_id, :].detach().clone()  # 送入actor的z不带梯度
...
class MAPPOTrainer:
    ...
    def ppo_update(self, sample, update_actor=True, ae=None, data4z=None):
        ...
        z_g, z_l = ae.encode(*data4z)  # (meta_batch, z_dim) & (meta_batch, n_agents, z_dim)
    ...
    def save_model(self, save_path):
        # save_path = os.path.join(save_path, "agent_%d.pkl" % self.agent_id)
    ...
    def load_model(self, load_path):
        # load_path = os.path.join(load_path, "agent_%d.pkl" % self.agent_id)
