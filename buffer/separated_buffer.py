...
class SeparatedReplayBuffer(object):
    def __init__(self, cfg, obs_space, share_obs_space, act_space):
        ...
        self._use_gae = cfg.use_gae  # 0.95
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        sampler = [rand[i * mini_batch_size:(i + 1) * mini_batch_size] for i in range(num_mini_batch)]  # 样本的indices

        share_obs = self.share_obs[:-1].reshape(-1, *self.share_obs.shape[2:])
        obs = self.obs[:-1].reshape(-1, *self.obs.shape[2:])
        # rewards = self.rewards[:-1].reshape(-1, *self.rewards.shape[2:])  # 09.13 change
        # next_obs = self.obs[1:].copy().reshape(-1, *self.obs.shape[2:])
        ...
        for indices in sampler:
            ...
            # rewards_batch = rewards[indices]  # 09.13 change
